...
-- Purpose: Validates if the insurance record exists for a patient.
-- If yes → update, if not → create.
...
-- Purpose: Automatically logs a note in a new table called EmergencyVisitLog when a patient visits the emergency room more than once, indicating a potential chronic or recurring condition.
...
-- Purpose: When a doctor’s schedule is removed (e.g., leaves or changes department), this trigger automatically revokes their access to patient histories they were allowed to view.
