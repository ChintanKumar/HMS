...
CREATE TABLE Patient (
    email VARCHAR(50) PRIMARY KEY,                      -- Primary key: unique email for each patient
    password VARCHAR(30) NOT NULL,                      -- Patient's login password
...
CREATE TABLE MedicalHistory (
    id INT PRIMARY KEY AUTO_INCREMENT,                  -- Primary key: auto-generated history ID
    date DATE NOT NULL,                                 -- Record creation date
    conditions VARCHAR(255) NOT NULL,                   -- Medical conditions
    surgeries VARCHAR(255) NOT NULL,                    -- Surgical history
    medication VARCHAR(255) NOT NULL                    -- Current medications
);

-- Creating Doctor table
CREATE TABLE Doctor (
    email VARCHAR(50) PRIMARY KEY,                      -- Primary key: unique email for each doctor
    password VARCHAR(30) NOT NULL,                      -- Doctor's login password
...
CREATE TABLE Appointment (
    id INT PRIMARY KEY AUTO_INCREMENT,                  -- Primary key: auto-generated appointment ID
    date DATE NOT NULL,                                 -- Appointment date
    starttime TIME NOT NULL,                            -- Appointment start time
    endtime TIME NOT NULL,                              -- Appointment end time
...
CREATE TABLE PatientsAttendAppointments (
    patient VARCHAR(50) NOT NULL,                       -- Foreign key: patient email
    appt INT NOT NULL,                                  -- Foreign key: appointment ID
    concerns VARCHAR(255) NOT NULL,                     -- Patient's stated concerns
    symptoms VARCHAR(255) NOT NULL,                     -- Reported symptoms
...
CREATE TABLE RoomsAndWards (
    RoomID INT PRIMARY KEY AUTO_INCREMENT,              -- Primary key: unique room ID
    RoomType ENUM('General', 'ICU', 'Private') NOT NULL,-- Type of room
    AvailabilityStatus ENUM('Available', 'Occupied')    -- Room availability
                       DEFAULT 'Available',
    AssignedPatientID VARCHAR(50),                      -- Foreign key: assigned patient email
...
CREATE TABLE Diagnose (
    appt INT NOT NULL,                                  -- Foreign key: appointment ID
    doctor VARCHAR(50) NOT NULL,                        -- Foreign key: doctor email
    diagnosis VARCHAR(255) NOT NULL,                    -- Diagnosis summary
    prescription VARCHAR(255) NOT NULL,                 -- Prescribed medication
    FOREIGN KEY (appt) REFERENCES Appointment (id) ON DELETE CASCADE,
...
CREATE TABLE SupportStaff (
    StaffID INT PRIMARY KEY AUTO_INCREMENT,             -- Primary key: unique staff ID
    ...
    Shift ENUM('Morning', 'Evening', 'Night')           -- Working shift
);
...
CREATE TABLE Emergency (
    EmergencyID INT PRIMARY KEY AUTO_INCREMENT,         -- Primary key: unique emergency case ID
    PatientID VARCHAR(50) NOT NULL,                     -- Foreign key: patient email
    PatientCondition TEXT NOT NULL,                     -- Condition details
    DoctorID VARCHAR(50) NOT NULL,                      -- Foreign key: attending doctor email
    ArrivalTime DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- Timestamp of arrival
...
CREATE TABLE Insurance (
    InsuranceID INT PRIMARY KEY AUTO_INCREMENT,         -- Primary key: insurance record ID
    PatientID VARCHAR(50) NOT NULL,                     -- Foreign key: patient email
    ProviderName VARCHAR(255) NOT NULL,                 -- Insurance company name
...
CREATE TABLE Schedule (
    id INT PRIMARY KEY AUTO_INCREMENT,                  -- Primary key: schedule ID
    starttime TIME NOT NULL,                            -- Start time of schedule
    endtime TIME NOT NULL,                              -- End time of schedule
    breaktime TIME NOT NULL,                            -- Break period during schedule
    day ENUM('Monday', 'Tuesday', 'Wednesday', 
             'Thursday', 'Friday', 'Saturday', 'Sunday') NOT NULL  -- Day of the week
);

-- Creating DoctorViewsHistory table (junction table)
CREATE TABLE DoctorViewsHistory (
    history INT NOT NULL,                               -- Foreign key: medical history ID
    doctor VARCHAR(50) NOT NULL,                        -- Foreign key: doctor email
    FOREIGN KEY (doctor) REFERENCES Doctor (email) ON DELETE CASCADE,
...
CREATE TABLE BillingAndPayments (
    billing_id INT PRIMARY KEY AUTO_INCREMENT,          -- Primary key: billing record ID
    patient_id VARCHAR(50) NOT NULL,                    -- Foreign key: patient email
    appointment_id INT NOT NULL,                        -- Foreign key: appointment ID
    total_amount DECIMAL(10,2) NOT NULL,                -- Total amount billed
    paid_amount DECIMAL(10,2) DEFAULT 0.00,             -- Amount paid by patient
    payment_method ENUM('Cash', 'Card', 'Insurance', 
                        'Online') NOT NULL,             -- Method of payment
...
CREATE TABLE DocsHaveSchedules (
    sched INT NOT NULL,                                 -- Foreign key: schedule ID
    doctor VARCHAR(50) NOT NULL,                        -- Foreign key: doctor email
...
CREATE TABLE MedicineInventory (
    medicine_id INT PRIMARY KEY AUTO_INCREMENT,         -- Primary key: medicine ID
    ...
    category ENUM('Tablet', 'Capsule', 'Syrup', 
                  'Injection', 'Ointment') NOT NULL,    -- Medicine form/category
    quantity INT NOT NULL DEFAULT 0,                    -- Quantity in stock
    unit_price DECIMAL(10,2) NOT NULL,                  -- Price per unit
    expiry_date DATE                                    -- Expiry date of medicine
);
...
-- USE CASE: Books an appointment for a patient with a doctor.
DELIMITER //
...
-- USE CASE: Updates a patient’s billing record after receiving a payment.
DELIMITER //
...
-- PROCEDURE: GenerateBillingStatement
-- USE CASE: Generates a billing statement for a given appointment, including patient details, diagnosis, and cost breakdown.
DELIMITER //
...
BEGIN
    ...
    UNION ALL
    SELECT 'Medication Cost (if applicable)', 0.00
    UNION ALL
    SELECT '--- END OF STATEMENT ---', NULL;
END //
DELIMITER ;

-- PROCEDURE: TransferPatientRoom
-- USE CASE: Transfers a patient to a new room by updating the RoomsAndWards table and releasing the previous room.
DELIMITER //
...
-- PROCEDURE: AdmitEmergencyPatient
-- USE CASE: Handles emergency admission by logging details and assigning an ICU room.
DELIMITER //
...
-- PROCEDURE: UpdateOrCreateInsuranceRecord
-- USE CASE: Updates an existing insurance record or creates a new one for a patient.
DELIMITER //
...
-- PROCEDURE: GenerateFinalBill
-- USE CASE: Calculates final bill by summing unpaid charges and applying insurance coverage.
DELIMITER //
...
BEGIN
    ...
    IF has_insurance > 0 THEN
        SET unpaid_total = unpaid_total * 0.8;
    END IF;
...
-- PROCEDURE: DischargePatient
-- USE CASE: Finalizes billing and room release during patient discharge.
DELIMITER //
...
-- USE CASE: Fetches a doctor's availability on a given day.
DELIMITER //
...
-- FUNCTION: GetMedicineStockLevel
-- USE CASE: Retrieves the current quantity available for a specific medicine.
DELIMITER //
...
-- USE CASE: Calculates the total outstanding balance for a patient.
DELIMITER //
...
-- FUNCTION: GetPatientEmergencyVisitCount
-- USE CASE: Returns the number of emergency room visits made by a patient.
DELIMITER //
...
-- USE CASE: Verifies if a patient has an active insurance record.
DELIMITER //
...
-- FUNCTION: GetDoctorUtilization
-- USE CASE: Calculates the percentage of scheduled time used by a doctor for appointments.
DELIMITER //
...
-- USE CASE: Computes the unpaid balance for a patient’s account.
DELIMITER //
...
-- TRIGGER: trg_UpdateRoomAvailability
-- USE CASE: Automatically updates room availability status when a patient is assigned or discharged.
DELIMITER //
...
-- TRIGGER: trg_AutoCompleteAppointments
-- USE CASE: Automatically marks past appointments as 'Completed' if still 'Scheduled'.
DELIMITER //
...
-- TRIGGER: PreventExpiredMedicineDispensing
-- USE CASE: Prevents reduction in stock for expired medicines.
DELIMITER //
...
-- TRIGGER: UpdatePaymentStatus
-- USE CASE: Automatically updates payment status based on amount paid.
DELIMITER //
...
-- TRIGGER: after_emergency_insert_log_returning_patient
-- USE CASE: Logs entries for repeat emergency room visits.
DELIMITER //
...
-- TRIGGER: after_schedule_delete_cleanup_access
-- USE CASE: Revokes doctor's access to history records when schedule is deleted.
DELIMITER //
...
-- TRIGGER: BeforeInsert_BillingValidation
-- USE CASE: Prevents overpayment by validating paid amount during insertion.
DELIMITER $$
...
-- TRIGGER: AfterUpdate_PaymentCompletedLog
-- USE CASE: Logs successful payment completion.
DELIMITER $$
...
-- QUERY: Find all available rooms
SELECT 
    RoomID,
    RoomType,
    AvailabilityStatus
...
-- QUERY: Patients and their insurance details
SELECT 
    p.name AS PatientName,
    p.email AS PatientEmail,
...
-- QUERY: Medicines expiring in the next 6 months
SELECT 
    medicine_name, 
    batch_number, 
    manufacturer, 
...
-- QUERY: Total billed and paid amount per patient
SELECT 
    p.name AS PatientName, 
    SUM(bp.total_amount) AS TotalBilled, 
    SUM(bp.paid_amount) AS TotalPaid
FROM 
    Patient p
JOIN 
    BillingAndPayments bp ON p.email = bp.patient_id
GROUP BY 
    p.name;

-- QUERY: Medicines with low stock (<20 units)
SELECT 
    m.medicine_id,
    m.medicine_name,
...
FROM 
    MedicineInventory m
WHERE 
    GetMedicineStockLevel(m.medicine_name) < 20
ORDER BY 
    m.expiry_date ASC;

-- QUERY: Total outstanding dues from unpaid bills
SELECT 
...
-- QUERY: Fetch patients with medical history and assigned rooms
SELECT 
    p.name AS PatientName,
    p.email AS PatientEmail,
...
-- QUERY: Doctors with schedules and appointments
SELECT 
...
-- QUERY: Billing details with patient and appointment info
SELECT 
...
-- QUERY: Support staff with most shifts in Emergency dept
SELECT 
    ss.Name, 
    ss.Role, 
    ss.Department, 
    COUNT(ss.Shift) AS ShiftCount
FROM 
    SupportStaff ss
WHERE 
    ss.Department = 'Emergency'
GROUP BY 
    ss.Name, ss.Role, ss.Department
HAVING 
    COUNT(ss.Shift) = (
...
-- QUERY: Doctors with most emergency cases
SELECT 
    d.name AS DoctorName, 
    COUNT(e.PatientID) AS EmergencyCases
FROM 
    Doctor d
JOIN 
    Emergency e ON d.email = e.DoctorID
GROUP BY 
    d.name
HAVING 
    COUNT(e.PatientID) = (
...
-- QUERY: Patients with highest billing amounts
SELECT 
    p.name AS PatientName, 
    SUM(bp.total_amount) AS TotalBilling
FROM 
    Patient p
JOIN 
    BillingAndPayments bp ON p.email = bp.patient_id
...
-- QUERY: Appointments and prescribed medications per patient
SELECT 
    p.name AS PatientName,
    a.date AS AppointmentDate,
    d.name AS DoctorName,
    ...
    dg.prescription
FROM 
    PatientsAttendAppointments paa
JOIN 
    Appointment a ON paa.appt = a.id
JOIN 
    Patient p ON paa.patient = p.email
JOIN 
    Diagnose dg ON a.id = dg.appt
JOIN 
    Doctor d ON dg.doctor = d.email
ORDER BY 
    a.date DESC;

-- QUERY: Doctors who viewed patient histories with specific prescriptions
SELECT DISTINCT 
    d.name AS DoctorName,
    d.email AS DoctorEmail,
    ...
    mh.medication
FROM 
    DoctorViewsHistory dvh
JOIN 
    Doctor d ON dvh.doctor = d.email
JOIN 
    MedicalHistory mh ON dvh.history = mh.id
WHERE 
    mh.medication IN (
...
-- QUERY: Top 5 insurance providers by billing amount
SELECT 
    i.ProviderName,
...
FROM 
    Insurance i
JOIN 
    Patient p ON i.PatientID = p.email
JOIN 
    BillingAndPayments bp ON p.email = bp.patient_id
GROUP BY 
    i.ProviderName
...
-- QUERY: Patients with pending payments and outstanding dues
SELECT 
    p.email AS PatientEmail,
    p.name AS PatientName,
...
FROM 
    Patient p
WHERE 
    CalculateBalance(p.email) > 0;

-- QUERY: Room occupancy status with patient info
SELECT 
    r.RoomID, 
    r.RoomType, 
...
FROM 
    RoomsAndWards r
LEFT JOIN 
    Patient p ON r.AssignedPatientID = p.email;

-- QUERY: Patients with latest appointment info
SELECT 
    p.email AS PatientEmail, 
    p.name AS PatientName, 
    a.date AS AppointmentDate, 
...
FROM 
    Patient p
LEFT JOIN 
    PatientsAttendAppointments paa ON p.email = paa.patient
LEFT JOIN 
    Appointment a ON paa.appt = a.id
ORDER BY 
    a.date DESC;
